...
case '#stickff':
	...
	case '#sticker2':
            ...
            const cariwallu = await fetch(`https://api.zeks.xyz/api/searchsticker?apikey=apivinz&q=${body.slice(10)}`)
            const nihbang = await cariwallu.json()
			const putri = nihbang.sticker
			let nihnya = putri[Math.floor(Math.random() * putri.length)]
            await client.sendStickerfromUrl(dari, nihnya)
            .catch(() => {
