...
*Provider :* INDIHOME ASU`, id)
	...
	case '#sticktahta':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
			...
			client.reply(dari, mess.wait, id)
			const tahtalu = arg[1]
	        client.sendStickerfromUrl(dari, `https://api.vhtear.com/hartatahta?text=${tahtalu}&apikey=JanganDiGantiLagi`)
			break
	case '#stickwolf':
		if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
...
case '#stickff':
	...
	case '#stickgoogle':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
		 ...
		 client.reply(dari, mess.wait, id)
		 arg = body.trim().split('|')
		 const googlenyas = `https://api.vhtear.com/googletext?text1=${arg[1]}&text2=${arg[2]}&text3=${arg[3]}&apikey=JanganDiGantiLagi`
	...
	case '#customtahta':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
            arg = body.trim().split('|')
			client.reply(dari, mess.wait, id)
			 const noon = `https://naufalhoster.xyz/textmaker/hartatahta_custom?apikey=JanganDiGantiLagi&text1=${arg[1]}&text2=${arg[2]}&text3=${arg[3]}`
	...
	case '#stickerml':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
	     ...
	     arg = body.trim().split('|')
		 const textmls = arg[2]
		 const mlnyas = `https://api.vhtear.com/logoml?hero=arg[1]&text=${textmls}&apikey=JanganDiGantiLagi`
    ...
    case '#gs':
		...
		if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10)  {
                    ...
                    await exec(`gify ${filename} ./media/output.gif --fps=60 --scale=240:240`, async function (error, stdout, stderr) {
                        const gif = await fs.readFileSync('./media/output.gif', { encoding: "base64" })
                        await client.sendImageAsSticker(dari, `data:image/gif;base64,${gif.toString('base64')}`, { author: 'BennyBOT', pack: 'BOT', keepScale: 'true' })
                    })
                } else (
	...
	if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
				if (args.length === 1) return client.reply(dari, 'Teksnya mana??', id)
	    client.reply(dari, mess.wait, id)
		const tahtanya = `https://api.vhtear.com/hartatahta?text=${body.slice(7)}&apikey=JanganDiGantiLagi`
	...
	case '#teksmatrix':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
			...
			client.reply(dari, mess.wait, id)
			const matrixnya = `https://naufalhoster.xyz/textmaker/matrix?apikey=JanganDiGantiLagi&text=${body.slice(12)}`
	...
	case '#tekspetir':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
			...
			const petirnya = `https://api.vhtear.com/thundertext?text=${textnye}&apikey=JanganDiGantiLagi`
	...
	case '#teksph':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
          ...
          arg = body.trim().split('|')
			client.reply(dari, mess.wait, id)
		 const phnya = `https://naufalhoster.xyz/textmaker/pornhub?apikey=JanganDiGantiLagi&text1=${arg[0]}&text2=${arg[1]}`
	...
	case '#ttg':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
                 client.reply(dari, mess.wait, id)
                await client.sendStickerfromUrl(dari, `https://api.vhtear.com/textxgif?text=${body.slice(5)}&apikey=JanganDiGantiLagi`)
     ...
     case '#emojistiker':
		       ...
		       if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
                client.reply(dari, mess.wait, id)
                    ...
                    client.sendStickerfromUrl(dari, "https://api.vhtear.com/emojitopng?code=" + bjbjbja + "&apikey=" + "JanganDiGantiLagi")
                    break
	case '#textff':
	case '#teksff':
	    if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
	...
	case '#textgoogle':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
		 ...
		 client.reply(dari, mess.wait, id)
		 arg = body.trim().split('|')
		 const googlenya = `https://api.vhtear.com/googletext?text1=${arg[1]}&text2=${arg[2]}&text3=${arg[3]}&apikey=JanganDiGantiLagi`
	...
	case '#teksml':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
		 ...
		 client.reply(dari, mess.wait, id)
		 arg = body.trim().split('|')
		 const mlnya = `https://api.vhtear.com/logoml?hero=${arg[1]}&text=${arg[2]}&apikey=JanganDiGantiLagi`
	...
	case '#teksdarah':
		...
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
         if (args.length === 1) return client.reply(dari, 'Teks nya mana??', id)
		 client.reply(dari, mess.wait, id)
		 const darah = `https://naufalhoster.xyz/textmaker/blood?apikey=JanganDiGantiLagi&text=${body.slice(11)}`
    ...
    /* case '#bokep':
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
          client.reply(dari, mess.wait, id)
          const bokap = await axios.get(`https://api.vhtear.com/xxxsearch?query=${body.slice(7)}&apikey=JanganDiGantiLagi`)
...
Contoh: ${prefix}setprefix #`, id)
	...
	if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
			if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
			    const cersexnya = await fetch(`https://api.vhtear.com/cerita_sex&apikey=JanganDiGantiLagi`)
	...
	if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
	    if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
		      	const puisinya = await fetch('https://api.vhtear.com/puisi_image&apikey=JanganDiGantiLagi')
	...
	case '#tiktok':
         ...
         if (args.length === 1) return client.reply(dari, 'Kirim perintah *#tiktok [linkTiktok]*\nContoh : *#tiktok https://vt.tiktok.com/yqyjPX/*', id)
                        client.reply(dari, mess.wait, id)
                        const tikt = await fetch(`https://api.vhtear.com/tiktokdl?link=${body.slice(8)}&apikey=JanganDiGantiLagi`)
...
➸ *Wilayah:* +${region_name}`
     ...
     break
      case '#play':
				...
				if (args.length == 1) return client.reply(dari, `Untuk mencari lagu from youtube\n\nPenggunaan: #play judul lagu`, id)
                        client.reply(dari, mess.wait, id)
                        const webplay = await fetch(`https://api.vhtear.com/ytmp3?query=${serplay}&apikey=JanganDiGantiLagi`)
	...
	case '#ytsearch':
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
		 ...
		 const searchnya = await fetch(`https://api.vhtear.com/youtube?query=${linknye}&apikey=JanganDiGantiLagi`)
	...
	if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
		if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
	    const speknya = await fetch(`https://api.vhtear.com/gsmarena?query=${body.slice(8)}&apikey=JanganDiGantiLagi`)
...
*Link*: ${url}`
	...
	if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
    if (args.length == 1) return client.reply(dari, 'Kirim perintah *#lirik [optional]*, contoh *#lirik aku bukan boneka*', id)
	client.sendFileFromUrl(dari, `https://api.vhtear.com/write?text=${body.slice(12)}&apikey=JanganDiGantiLagi`, 'nulis.jpg', 'Neh..', id)
	break
    case '#daftar':
...
Total user premium : ${_premium.length}
   ...
   if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
            client.sendLinkWithAutoPreview(dari, 'https://github.com/bennyganteng/JanganDiGantiLagi', info)
            break
        case '#snk':
		if (isAfk) return await client.reply(dari, `Maaf ${pushname}, Kamu sedang afk! Ketik #unafk untuk mematikan`, id)
		if (!isRegistered) return await client.reply(dari, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n *#daftar*`, id)
